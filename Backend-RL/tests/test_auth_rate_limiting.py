...
def test_invalid_api_key():
    response = client.get("/api/demand/skus", headers={"X-API-Key": "wrong-key"})
    assert response.status_code == 401
    assert "Invalid API Key" in response.json()["detail"]

def test_valid_api_key():
    # Will return 400 because no file uploaded yet, but NOT 401
    response = client.get("/api/demand/skus", headers={"X-API-Key": "test-secret-key"})
    assert response.status_code != 401
...
def test_rate_limiting():
    ...
    hit_429 = False
    for i in range(105):
        response = client.get("/nonexistent-endpoint", headers={"X-API-Key": "test-secret-key"})
