...
print("Loading Dataset 1 (Retail Store Inventory)...")
raw_df = pd.read_csv(HERE / "data/retail_store_inventory.csv")
raw_df['Date'] = pd.to_datetime(raw_df['Date'])
raw_df.rename(columns={"Units Sold": "Demand", "Product ID": "sku"}, inplace=True)

top_skus = raw_df.groupby('sku')['Demand'].sum().nlargest(4).index.tolist()
...
for sku in top_skus:
    ...
    temp_csv = HERE / f"data/temp_sku_{sku}.csv"
...
axes[0].set_xticks(x)
axes[0].set_xticklabels(top_skus)
...
axes[1].set_xticks(x)
axes[1].set_xticklabels(top_skus)
axes[1].set_title('Total Reward across SKUs')
axes[1].legend()

plt.tight_layout()
plt.savefig(PLOTS_DIR / "replenix_dataset1_multisku_metrics.png", dpi=150)
plt.close()

with open(HERE / "results/dataset1_multisku_results.json", "w") as f:
