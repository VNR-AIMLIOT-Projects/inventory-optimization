...
class RobustInventoryEnvironment(InventoryEnvironment):
    def __init__(self, historical_data, **kwargs):
        ...
        # Override action space with a pseudo-logarithmic scale to handle extreme spikes
        # Allows fine control at low quantities, and large jumps for massive restocks
        self.action_space = [0, 5, 10, 20, 50, 100, 200, 350, 500, 750, 1000, 1500, 2500, 4000, 6000]
        self.action_size = len(self.action_space)

def run_robust_training(dataset_num=2, episodes=1500):
    print("=" * 60)
    print(f"  REPLENIX ROBUST TRAINING - DATASET {dataset_num}")
    print("=" * 60)
    ...
    print(f"Selected Top 2 SKUs: {top_skus}")
    
    results = []
    
    for sku in top_skus:
        print("\n" + "*" * 50)
        print(f"*** STARTING ROBUST TRAINING FOR SKU: {sku} ***")
        print("*" * 50)
        ...
        temp_csv = data_dir / f"temp_robust_sku_{sku}.csv"
        ...
        # Implementing the 4 robust fixes:
        # 1. Lower holding cost so it's not penalized severely for carrying safety stock.
        # 2. Huge stockout penalty to force it to meet demand spikes.
        # 3. High price to ensure positive reward for selling.
        # 4. Action space handled by the Subclass above.
        ...
        # We increase Replay Buffer size and batch size via DQNAgent constructor if possible, 
        # else rely on the longer training epochs (1500)
        agent = DQNAgent(state_size, action_size, total_episodes=episodes)
        
        print(f"-> Robust Environment Ready.")
        print(f"   Holding Cost: {env.h} | Stockout Pen: {env.c_stockout} | Price: {env.p}")
        ...
        for ep in range(episodes):
            ...
            if (ep + 1) % 250 == 0 or ep == 0:
                print(f"Ep {ep+1:<7} | Total Reward: {ep_r:<15.0f} | Epsilon: {agent.epsilon:<10.3f}")
        ...
        ev_env = RobustInventoryEnvironment(processed_df, **env_kwargs)
        ...
        total_sold = sum([d['units_sold'] for d in dqn_log])
        total_demand = sum([d['demand'] for d in dqn_log])
        dqn_sl = total_sold / total_demand if total_demand > 0 else 1.0
        ...
        results.append({
            "sku": sku,
            "service_level": dqn_sl,
            "reward": dqn_r
        })

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--dataset", type=int, default=2, choices=[1, 2], help="Dataset 1 (Retail) or 2 (UCI Wholesale)")
    parser.add_argument("--episodes", type=int, default=1500, help="Number of training episodes")
    args = parser.parse_args()
    
    run_robust_training(dataset_num=args.dataset, episodes=args.episodes)
